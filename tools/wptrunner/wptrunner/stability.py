...
def write_github_checks_summary_inconsistent(log, inconsistent, iterations):
    ...
    log("These may be pre-existing or new flakes. Please try to reproduce (see "
        "the above WPT command for guidance) and determine if your change "
        "introduced the flake. If you are unable to reproduce the problem, "
        "please tag @web-platform-tests/wpt-core-team in a comment for help.\n")
